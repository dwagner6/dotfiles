...
static void initialize_display(uint16_t vout, char *vout_str, char *imon_str);
...
void configure_power_supply()
{
    ...
    initialize_display(VOUT_DEFAULT_MV, vout_str, imon_str);
}
...
void power_supply_fsm()
{
    ...
    static enum {RESET, OFF, ON, STANDBY} state = RESET;
    ...
    switch(state)
    {
        ...
        case STANDBY:
            lcd_command(LCD_CMD_CLR_DISP);
            lcd_command(LCD_CMD_GOTO | LCD_R1);
            lcd_print_string("    STANDBY     ");
            lcd_command(LCD_CMD_GOTO | LCD_R2);
            lcd_print_string("   EN TO WAKE   ");
            if(!lcd_pending_commands())
                standby_mode();
            state = OFF;
            initialize_display(vou)
            break;
    }
    loop_cnt++;
}
