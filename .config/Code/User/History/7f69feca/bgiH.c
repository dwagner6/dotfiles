...
void int_to_hex(uint8_t num_digits, uint32_t value, char * hex_string);

int main()
{
    configure_systick();
    configure_watchdog(1000000);
    configure_lcd();
    configure_keypad();
    ...
    while(1)
    {
        ...
        if(get_key(&key))
        {
            *(row1_chars + row1_pos) = key;
            ...
            int_to_hex(NUM_FIELD_LENGTH - 1, row1_value, row2_chars);
...
void int_to_hex(uint8_t num_digits, uint32_t value, char * hex_string)
{
    ...
    for(uint8_t i = num_digits - 1; i >= 0; i--)
    {
