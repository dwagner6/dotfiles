    ...
    while(1)
    {
        ...
        if(get_key(&key))
        {
            *(row1_chars + row1_pos) = key;
            ...
            uint8_t hex_digit = (row1_value >> 4*0) & 0xf;
            *(row2_chars+0) = hex_digit+( (hex_digit < 10)?'0':'A'-10);
            hex_digit = (row1_value >> 4*1) & 0xf;
            *(row2_chars+1) = hex_digit+( (hex_digit < 10)?'0':'A'-10);
            hex_digit = (row1_value >> 4*2) & 0xf;
            *(row2_chars+2) = hex_digit+( (hex_digit < 10)?'0':'A'-10);
            hex_digit = (row1_value >> 4*3) & 0xf;
            *(row2_chars+3) = hex_digit+( (hex_digit < 10)?'0':'A'-10);
