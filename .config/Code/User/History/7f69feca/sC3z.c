    ...
    while(1)
    {
        ...
        if(get_key(&key))
        {
            ...
            if(key == '#')
            {
                ...
                row1_pos = NUM_FIELD_END;
                continue;
            }
 */
            *(row1_chars + row1_pos) = key; // save key to array
            lcd_putchar(key);               // display key in right position
            row1_pos++;
            ...
            static uint32_t row1_value = str_to_uint32(row1_chars);
