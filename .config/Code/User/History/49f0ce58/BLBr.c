...
* FILENAME :        lcd.c          
*
* DESCRIPTION :     Driver implementation for NHD-021K1Z wired in 4-bit mode
*
...
static uint32_t next_tail() { return (tail + 1 == LCD_FIFO_SIZE) ? 0 : tail + 1; }
static uint32_t next_head() { return (head + 1 == LCD_FIFO_SIZE) ? 0 : head + 1; }
...
void uint16_to_fixed_str(uint16_t value, char *string) 
{
    ...
    while (temp_value != 0) 
    {
        temp_str[length++] = '0' + mod10(temp_value);
        temp_value = div10(temp_value);
    }
