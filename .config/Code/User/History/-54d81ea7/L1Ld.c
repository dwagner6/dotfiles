...
uint16_t convertToMillivolts(uint16_t adcValue);
...
int main()
{
    ...
    adc -> set_cs = ADC_CS_EN(1);   // enable adc 

    while( !(adc -> cs && ADC_CS_READY_MASK) )
        continue; 

    adc -> set_cs = ADC_CS_AINSEL(0);  // adc channel set to GPIO26 before conversion starts
...
uint16_t convertToMillivolts(uint16_t adcValue) {
    // Calculate the voltage step size in millivolts per ADC count (using power of 2)
    uint32_t voltageStep = (VOLTAGE_RANGE << 8) / ADC_RESOLUTION; // Shift by 8 (2^8 = 256)
    
    // Convert ADC value to millivolts (using 8-bit precision for fixed-point arithmetic)
    uint32_t voltage = (uint32_t)adcValue * voltageStep; // Multiply without loss of precision
    
    // Right-shift by 8 to get the millivolt value
