...
int main()
{
    ...
    while(1)
    {
        ...
        if(++cnt > 500)
        {
            ...
            cnt = 0;
            uint16_t samples_waiting = adc -> fcs & ADC_FCS_LEVEL_MASK;
            uint8_t conversion_error = adc -> fcs & ADC_FCS_ERR_MASK;
            uint16_t adc_fifo_first_sample = adc -> fifo & ADC_FIFO_VAL_MASK;
            print_string("Samples waitings: ");
            print_hex(4, samples_waiting);
            print_string("\n\r");
            print_string("ADC FIFO sample 0: ");
            print_hex(4, adc_fifo_first_sample);
            print_string("\n\r");
            print_string("Conversion error: ");
            print_hex(2, conversion_error);
            print_string("\n\r");
