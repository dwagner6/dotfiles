...
uint16_t adc_to_millivolts(uint16_t adc_value) {
    ...
    uint32_t voltage_step = (VOLTAGE_RANGE << 8) / ADC_RESOLUTION; // Shift by 8 (2^8 = 256)
    
    // Convert ADC value to millivolts (using 8-bit precision for fixed-point arithmetic)
    uint32_t voltage = (uint32_t)adc_value * voltage_step; // Multiply without loss of precision
