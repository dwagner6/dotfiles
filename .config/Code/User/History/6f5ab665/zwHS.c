...
void configure_pin27( void )
{
    ...
    pads_bank0 -> GPIO(27)       = out_pads_bank0_gpio;
	io_bank0 -> GPIO_CTRL(27)   = out_io_bank0gpio_ctrl;

    register_gpio_irq_callback(27, callback);
    io_bank0 -> proc0_inte3 |= (IO_BANK0_PROC0_INTE3_GPIO27_LEVEL_HIGH_MASK
                                | IO_BANK0_PROC0_INTE3_GPIO27_LEVEL_LOW_MASK);
}

static void callback()
{
    if( io_bank0 -> proc0_ints3 & IO_BANK0_PROC0_INTS3_GPIO27_LEVEL_HIGH_MASK)
        pin_state = true;            
    if( io_bank0 -> proc0_ints3 & IO_BANK0_PROC0_INTS3_GPIO27_LEVEL_LOW_MASK)
        pin_state = false;
}
