...
* FILENAME :        adc.c         
*
* DESCRIPTION :     Driver implementation for ADC0 on GPIO26 of RPI Pico.
*                   Runs in free-running mode
*
...
#define ADC_0V_OFFSET_MV    14

static void __attribute__((isr)) adc_irq_handler();
...
void configure_adc()
{
    ...
    while(! (resets -> reset_done & RESETS_RESET_DONE_IO_BANK0_MASK))
        continue;
    while(! (resets -> reset_done & RESETS_RESET_DONE_PADS_BANK0_MASK))
        continue;
...
uint16_t adc_to_millivolts(uint16_t adc_value) 
{
    ...
    uint16_t voltage_calibrated = (voltage >> 16);
    if( voltage_calibrated <= 13)
        voltage_calibrated -= ADC_0V_OFFSET_MV;
    if(voltage_calibrated > 3300)
        voltage_calibrated = 0;
