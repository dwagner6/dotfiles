...
_Bool tps55289_set_vout(uint16_t vout_set_mv)
{
    ...
    const uint32_t vref_offset = 45;

    uint32_t vout_v = vout_set_mv;
    uint32_t vref_v = (vout_v * 100) / intfb_ratio;
    vref_v = (vref_v * 1000) - vref_offset;
    uint16_t ref_value = (uint16_t)(vref_v / vref_step);
