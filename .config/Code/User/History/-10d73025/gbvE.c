...
{
    const uint16_t Vref_offset = 45000; // 45 * 1,000 for fixed-point arithmetic
    const uint16_t Vref_multiplier = 5645; // 0.5645 * 10,000 for fixed-point arithmetic
    const uint16_t Vout_multiplier = 564; // 0.0564 * 10,000 for fixed-point arithmetic

    uint32_t Vout = (uint32_t)vout_set_mv * 10000; // Scale Vout by 10,000 (from millivolts to volts)
    
    // Calculate the REF value
    uint32_t ref_value = ((Vout / Vout_multiplier) - Vref_offset) * 10000 / Vref_multiplier;
