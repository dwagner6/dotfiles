...
_Bool tps55289_set_vout(uint16_t vout_set_mv)
{
    ...
    const uint32_t vref_offset = 450000;
    //const uint16_t max_ref_value = 2047;

    uint32_t vref = (uint32_t) vout_set_mv * vref_step - vref_offset;
    uint16_t ref_value = (uint16_t)(vref / intfb_ratio);
    //if(ref_value > max_ref_value)
        //ref_value = max_ref_value;
    ...
    trans[2] = ref_value & 0xff00 | I2C_STOP;
