...
uint16_t calculate_ref(uint32_t vout_mv) {
    ...
    // Calculate the REF value based on the voltage mapping
    uint32_t REF = ((Vout_mV - voltage_min) * REF_increment) / (voltage_max - voltage_min);
