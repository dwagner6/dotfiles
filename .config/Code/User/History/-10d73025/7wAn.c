...
{
    const uint32_t Vref_offset = 45000; // 45 * 1000 for fixed-point arithmetic
    const uint32_t Vref_multiplier = 5645; // 0.5645 * 10000 for fixed-point arithmetic
    const uint32_t Vout_multiplier = 564; // 0.0564 * 10000 for fixed-point arithmetic

    uint32_t Vout = vout_set_mv * 10000; // Scale Vout by 10,000 (from millivolts to volts)

    // Calculate the REF value
    uint32_t ref_value = ((Vout + Vref_offset) * Vout_multiplier) / 10000;
